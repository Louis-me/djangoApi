 ...
 $(function () {
    $("#nav-left ul li:eq(3) a").css("color", "red")
     ...
     $("#btn-modal-confirm").click(function () {
        $.rpc.req("../../../step_del","post",{"sid": id},function(resp){
            if (resp && resp["code"] == 0) {
                    alert("成功")
                     location.reload()
             } else {
    ...
    function NewStep() {
        ...
        var data = {"name": name, "find_type":  find_type, "element_info": element_info,"operate_type":operate_type,
         "extend": extend, "sort":sort, "cid": $("#cid").val()}
        $.rpc.req("../../../step_new", "post", data, function(resp){
            if (resp && resp["code"] == 0) {
                    alert("成功")
                     location.reload()
            }
     ...
     function EditStep() {
        ...
        var sort = $("#sort").val()
        var data = {"name": name, "find_type":  find_type, "element_info": element_info,"operate_type":operate_type,
         "extend": extend, "sort":sort, "sid": id}
        $.rpc.req("../../../step_edit","post",data,function(resp){
            if (resp && resp["code"] == 0) {
                    alert("成功")
                     location.reload()
              } else {
