 ...
 $(function () {
    $("#nav-left ul li:eq(5) a").css("color", "red")
     ...
     $("#btn-modal-confirm").click(function () {
        $.rpc.req("../../common_case_step_del","post",{"id": id},function(resp){
            if (resp && resp["code"] == 0) {
                    alert("成功")
                     location.reload()
             } else {
    ...
    function NewStep() {
        ...
        var data = {"name": name, "find_type":  find_type, "element_info": element_info,"operate_type":operate_type,
         "extend": extend, "sort":sort, "cc_id": $("#cc_id").val()}
        $.rpc.req("../../common_case_step_new", "post", data, function(resp){
            if (resp && resp["code"] == 0) {
                    alert("成功")
                     location.reload()
            }
     ...
     function EditStep() {
        ...
        var sort = $("#sort").val()
        var data = {"name": name, "find_type":  find_type, "element_info": element_info,"operate_type":operate_type,
         "extend": extend, "sort":sort, "ccs_id": id}
        $.rpc.req("../../common_case_step_edit","post",data,function(resp){
            if (resp && resp["code"] == 0) {
                    alert("成功")
                     location.reload()
              } else {
