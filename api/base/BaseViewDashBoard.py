...
class BaseViewDashBoard:
    ...
    def dashBoard_top10_task(request):
        rp = Report.objects.order_by()[1:10]
        data = []
        categories = []
        # [{name: '读请求',data:rtps
        passed = {"data": [], "name": "通过"}
    ...
    def dashBoard_top100_case_time(request):
        lt_ms200 = {"name": "小于200ms", "data": []}
        gt_ms200 = {"name": "大于200ms小于500ms", "data": []}
        gt_ms500 = {"name": "大于500ms", "data": [], "color": "red"}
        data = []
        rps = ReportItem.objects.order_by()[1:100]
        for i in rps:
            sum_time = int(i.sum_time.split("ms")[0])
            if sum_time < 200:
                lt_ms200["data"].append(sum_time)
            if 200 < sum_time < 500:
                gt_ms200["data"].append(sum_time)
            if sum_time > 500:
                gt_ms500["data"].append(sum_time)
        data.append(lt_ms200)
        data.append(gt_ms200)
        data.append(gt_ms500)
        result = {'code': 0, "data": data}
        return JsonResponse(result)
